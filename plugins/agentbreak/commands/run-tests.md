...
**If `mcp.enabled: true` in application.yaml (any mode — mock or proxy), you MUST run inspect before proceeding:**

```bash
agentbreak inspect
```

Do NOT skip this. Do NOT proceed to Step 6 without running inspect when MCP is enabled. The validate output will warn "MCP enabled but registry not found" — that is your signal to run inspect immediately, not to continue without it.

If inspect fails (connection refused, auth error, timeout), **do NOT silently disable MCP and continue**. Stop and ask the user:
...
**First, check for contextual queries** generated by `/agentbreak:create-tests`:

```bash
cat .agentbreak/queries.yaml 2>/dev/null
```

**If `.agentbreak/queries.yaml` exists**, use those queries. Read the file and send each query as a curl request:

For each LLM query in `queries.llm`:
```bash
curl -s -w "\n" http://localhost:5005/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer dummy" \
  -d '{"model":"gpt-4o","messages":[{"role":"user","content":"<query content from file>"}]}'
```

For each MCP query in `queries.mcp`:
```bash
curl -s -w "\n" http://localhost:5005/mcp \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":<N>,"method":"tools/call","params":{"name":"<tool from file>","arguments":<arguments from file>}}'
```

Send all requests (LLM and MCP) with `&` for parallelism, then `wait`.

**If no queries.yaml exists**, fall back to generic requests:
...
for i in {1..10}; do
  curl -s -w "\n" http://localhost:5005/v1/chat/completions \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer dummy" \
    -d "{\"model\":\"gpt-4o\",\"messages\":[{\"role\":\"user\",\"content\":\"Test request $i\"}]}" &
done
wait
```

If MCP is also in mock mode and no queries.yaml:
```bash
for i in {1..5}; do
  curl -s -w "\n" http://localhost:5005/mcp \
    -H "Content-Type: application/json" \
...
**If `.agentbreak/queries.yaml` exists**, use those contextual queries as the prompts you send to the agent. These were generated based on the agent's actual purpose and tools, so they'll exercise realistic code paths.
